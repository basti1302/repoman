...
Repoman.prototype.exec = function (command, opts, cb) {
  ...
  _.each(this.repos, function (repo, name) {
    ...
    if (selectByTags(repo) && selectByRegex(repo, name)) {
      var repoType = self._determineRepoType(repo);
      var _command =  (self.scms[repoType][command]) ? self.scms[repoType][command] : 'cd {workspace}/{name}; ' + command;
      var params   = { name: name, url: repo.url, workspace: dir };
