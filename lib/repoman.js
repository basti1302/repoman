...
function _addToConfig(self, repositoryName, newConfig, opts, cb) {
  ...
  if (fs.existsSync(CONFIG_FILE)) {
    ...
    if (existing[repositoryName]) {
      return cb(new Error('A configuration for ' + repositoryName + ' already exists, won\'t overwrite.'));
    }
    var configIncrement = {};
...
function _removeFromConfig(self, repositoryName, opts, cb) {
  ...
  if (fs.existsSync(CONFIG_FILE)) {
    ...
    if (!config[repositoryName]) {
      return cb(new Error('There is no entry in the configuration file for ' + repositoryName + ', so there is nothing to remove.'));
    }
