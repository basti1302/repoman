...
// repository name | branch name | uncommited changes? | unpushed commits?
function _report(args) {

  var opts = {
    // TODO Enable short option when we can print the whole report into one
    // line. Unfortunately, this will probably require a fork of bagofcli
    // or using a different shell execution lib or adding a different version
    // of exec(command, fallthrough, cb) to bagofcli via PR.
    // short: true
  };
  opts.regex = args.parent.regex;
  if (args.parent.tags) {
  ...
  repoman.exec(
    'printf "Branch     : " && git symbolic-ref --short --quiet HEAD && ' +
    'printf "Uncommitted: " && ((git diff-index --quiet HEAD -- && echo "Clean") || echo "Dirty") && ' +
    'printf "Unpushed   : " && (git log --branches --not --remotes --oneline | wc -l)',
    opts, bag.exit)
}
