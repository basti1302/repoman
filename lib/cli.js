...
function _add(args) {
  ...
  if (!name && type === 'git') {
    nameMatch = /^.*\/(.*)\.git/.exec(url);
    if (nameMatch) {
      name = nameMatch[1];
      console.log('You have given no --repository-name (or --name), I have tried to figure it out from the given --url and arrived at "' + name + '".');
    }
  }
  if (!name) {
    console.error('You have given no --repository-name (or --name) and I have not been able to figure it out from the given --url. Please provide the name explicitly.');
    process.exit(1);
  }
...
function _remove(args) {
  ...
  if (!name) {
    console.error('You need to provide the mandatory parameter --name (or --repository-name).');
    process.exit(1);
  }
