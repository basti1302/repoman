...
function _config(args) {
  ...
  if (args.githubUser || args.githubOrg || args.githubAuthUser || args.githubAuthPass) {
    opts.github = {
      ...
      org: args.githubOrg,
      auth: {
        user: args.githubAuthUser,
        pass: args.githubAuthPass
      }
    };
  }
...
function _exec(args,  execArgs) {
  ...
  var scmsFile = (process.platform === 'win32') ? 'scms-win32.json' : 'scms.json',
    ...
    config = JSON.parse(bag.cli.lookupFile(args.parent.configFile || '.repoman.json'));
...
function _clean(args) {
  ...
  var config = JSON.parse(bag.cli.lookupFile(args.parent.configFile || '.repoman.json')),
